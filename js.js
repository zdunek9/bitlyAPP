...
function getLink(){                             //generate a new link
...
async function getNewLink(){                //download shortlink from api
    try{
        const res = await fetch('https://api-ssl.bitly.com/v4/shorten', {
            method: "POST",
            headers: {
                'Authorization': 'twoj token API',     //Paste your API code
...
function enterNewLink(){                    //naming the important links element with a check to see if such a shortened link exists
...
function addNewLink(){                                              //when all is set, adding new elements to the DOM
...
function cpLink(){                                      //copying a link to the clipboard
    navigator.clipboard.writeText(shortLink.value)
    copyBTN.textContent = 'Copied!'
}
function allLinks(){                                    //shows a dashboard of links
...
const checkdel = e =>{                                   //Removes an element from html,array and localstorage
...
function emptyElement(){                            //adds/removes inscription when empty panel
...
function receiveElementsFromLocalStorage(){                     //gets items from localstorage
...
document.addEventListener("keyup",event => {                                    //add enter button 
    if (event.code === 'Enter' && !newNamePopUp.classList.contains('secondPage') && newLinkValue.value != '') {
